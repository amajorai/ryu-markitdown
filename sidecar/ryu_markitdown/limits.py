...
MAX_INPUT_BYTES = _env_int("RYU_MARKITDOWN_MAX_INPUT_BYTES", 200 * _MIB, minimum=_MIB)
...
# document is useful, a 500 is not. Must stay under the proxy's 10 MiB body cap.
MAX_OUTPUT_BYTES = _env_int("RYU_MARKITDOWN_MAX_OUTPUT_BYTES", 8 * _MIB, minimum=64 * 1024)

# Wall-clock ceiling for one parse. MarkItDown is much faster than the OCR-capable
# backends, but a 200 MiB spreadsheet or a 500-member archive is still minutes of
# work, and the contract pins one number across all four backends.
TIMEOUT_SECS = _env_int("RYU_MARKITDOWN_TIMEOUT_SECS", 600, minimum=10)

# Concurrent parses. The conversion is CPU-bound (pdfminer / openpyxl are pure
# Python), so a small number is the honest default.
MAX_WORKERS = _env_int("RYU_MARKITDOWN_MAX_WORKERS", 2, minimum=1)

# Members we will expand out of one archive, and total expanded bytes — a zip
# bomb is otherwise a trivial local DoS, and ZIP is a headline MarkItDown format.
MAX_ARCHIVE_MEMBERS = _env_int("RYU_MARKITDOWN_MAX_ARCHIVE_MEMBERS", 512, minimum=1)
MAX_ARCHIVE_BYTES = _env_int("RYU_MARKITDOWN_MAX_ARCHIVE_BYTES", 512 * _MIB, minimum=_MIB)
...
# meant to idle-stop.
MAX_JOBS = _env_int("RYU_MARKITDOWN_MAX_JOBS", 64, minimum=4)
