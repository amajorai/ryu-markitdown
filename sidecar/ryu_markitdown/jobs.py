...
class JobStore:
    ...
    def submit(self, path: Path, filename: str, options: dict[str, Any]) -> Job:
        # The snapshot reports the caller's ORIGINAL filename, not `path.name`:
        # on the blob form the file on disk is named after its sha256, which is
        # not something a user would recognise on a chip.
        job = Job(id=f"parse_{uuid.uuid4().hex[:16]}", filename=filename or path.name)
        with self._lock:
        ...
        threading.Thread(
            target=self._run,
            args=(job, path, filename, options),
            name=f"parse-{job.id}",
            daemon=True,
        ).start()
        return job
    ...
    def _run(self, job: Job, path: Path, filename: str, options: dict[str, Any]) -> None:
        ...
        try:
            ...
            def work() -> None:
                try:
                    outcome["result"] = parse_file(path, filename, options)
            ...
            while not done.wait(timeout=0.25):
                ...
                if time.time() >= deadline:
                    self._fail(
                        ...
                        "(raise RYU_MARKITDOWN_TIMEOUT_SECS for very large inputs)",
                    )
                    return
